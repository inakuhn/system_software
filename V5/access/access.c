...
void buf();
void work_queue();
int open_device(char *device_path, int access);
int close_device(int fd);
void lock();
void mutex();
void *start_thread_lock();
void *start_thread_mutex();
...
int main(int argc, char *argv[]){
    int c;
    while ((c = getopt(argc, argv, ":horwt:d:abkq:lm")) != -1) {
        switch(c) {
            case 'h':
                printf (" Aufruf: ./access [-h] [-o] [-w] [-r] [-t] [-d] [-a] [-b] [-q] [-l] [-m]");
                return EXIT_SUCCESS;
            case 'o':
                openclose();
                break;
...
void lock()
{
    ...
    while(i < 2)
    {
        err = pthread_create(&(tid[i]), NULL, &start_thread_lock, NULL);
        if (err != 0)
...
void mutex()
{
    ...
    while(i < 2)
    {
        err = pthread_create(&(tid[i]), NULL, &start_thread_mutex, NULL);
        if (err != 0)
            printf("\ncan't create thread :[%s]", strerror(err));
        else
            printf("\n Thread created successfully\n");
...
void *start_thread_lock()
{
	...
	int fd = open_device(LOCK, O_RDONLY);
	if (fd < 0) {
        printf("failed\n");
    } else {
        printf("succeed\n");
...
void *start_thread_mutex()
{
	...
	int fd = open_device(MUX, O_RDONLY);
	if (fd < 0) {
        printf("failed\n");
    } else {
        printf("succeed\n");
...
int open_device(char *device_path, int access)
{
	int fd = open(device_path,access);
